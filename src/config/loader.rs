//! 配置加载、多环境叠加、环境变量合并
//!
//! 加载流程：读取 `default.toml` → 叠加 `{env}.toml` → 反序列化 → 环境变量覆盖 → 校验。
...
pub fn load_shared(config_dir: &str, env: &str) -> Result<SharedConfig> {
    ...
    Ok(std::sync::Arc::new(parking_lot::RwLock::new(cfg)))
}

/// 读取并解析单个 TOML 文件为 `toml::Value`
fn read_toml(path: &Path) -> Result<Value> {
    let content = std::fs::read_to_string(path)
        .map_err(|e| GatewayError::Config(format!("读取配置文件 {} 失败: {e}", path.display())))?;
    toml::from_str(&content)
        .map_err(|e| GatewayError::Config(format!("解析配置文件 {} 失败: {e}", path.display())))
}
...
fn deserialize(value: &Value) -> Result<AppConfig> {
    let toml_str = toml::to_string(value)
        .map_err(|e| GatewayError::Config(format!("配置序列化失败: {e}")))?;
    toml::from_str::<AppConfig>(&toml_str)
        .map_err(|e| GatewayError::Config(format!("配置反序列化失败: {e}")))
}
